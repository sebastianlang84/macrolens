...
- `apps/web` nutzt jetzt zusätzlich `Ultracite`/`Biome` für `npm run check` und `npm run fix`; das bestehende `eslint`-Setup (`npm run lint`) bleibt parallel bestehen. Biome läuft app-lokal mit eigener `apps/web/.gitignore`, damit der Check im Unterverzeichnis stabil ist.
- Ultracite-Baseline 2026-03-21 bereinigt: `npm run check`, `npm run lint` und `npm run build` laufen in `apps/web` gruen. Beim Build bleibt nur ein bekannter Next.js-Hinweis zur Workspace-Root-Erkennung bestehen, weil im Repo-Root und in `apps/web` jeweils ein `package-lock.json` liegt.
...
- `RSI Score` basiert auf einem geglaetteten Divergenz-Score aus Preis- und RSI-Regressionen, ist um `50` zentriert und wird als eigenstaendige Indikatorreihe gerendert. Die Glaettung laeuft jetzt low-lag ueber DEMA statt einfacher EMA; Divergenzmarker nutzen weiter Score-Pivots, verlangen aber zusaetzlich Score-Verbesserung, sinnvolle Pivot-Abstaende und verwenden rohe RSI-Werte als Confirm, wenn diese an den Pivot-Daten verfuegbar sind. `RSI Score W` nutzt dafuer Monday-aggregierte Weekly-OHLC aus den taeglichen Yahoo-Candles.
